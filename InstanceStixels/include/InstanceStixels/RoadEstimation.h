...
public:
    ...
    void Initialize(const float camera_center_y, const float baseline,
            const float focal, const int rows,
            const int cols, const int max_dis,
            const float road_vdisparity_threshold = 0.2f);
    void Finish ();

    bool Compute(const std::vector<pixel_t>& im);
    bool Compute(pixel_t* d_im);
...
private:
    ...
    void ComputeCameraProperties(cv::Mat vDisp, const float rho,
            const float theta, float& horizonPoint,
            float& pitch, float& cameraHeight, float& slope) const;
    bool ComputeHough(uint8_t* vDisp, float& rho, float& theta,
            float& horizonPoint, float& pitch,
...
private:
    ...
    int      m_HoughAccumThr; // Minimum number of points to form a line
